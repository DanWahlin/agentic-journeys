...
- [ ] iOS: Keep `APIClientProtocol` source-compatible, so existing test doubles still compile: add the due-date variants with default implementations in a protocol extension instead of changing existing method signatures. Parse and format date-only values in the device's local calendar and time zone, never UTC, so `2026-10-15` shows as October 15 everywhere. `AddTodoView` and `TodoDetailView` have an optional date picker, list rows show the due date when set, and a unit test decodes a todo with and without `dueDate`.
