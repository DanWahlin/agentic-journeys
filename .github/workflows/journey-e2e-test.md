...
4. **Verify**: Run verification commands from the README
   - For n8n, do not immediately curl the UI root after `azd up`. First poll `$N8N_URL/healthz` for up to 5 minutes, then verify the UI root. Verify `N8N_WEBHOOK_URL` matches the deployed HTTPS URL and the Container App uses `/healthz` probes, `minReplicas: 1`, `maxReplicas: 1`, and Single revision mode.
